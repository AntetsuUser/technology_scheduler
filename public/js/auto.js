$(function()
{
    ...
    $(document).ready(function() 
    {
        ...
        // パーセントのthのクリックを取得
        $(".percent th").on("click", function () {
            let name = $(this).attr('name');
            let textAbove = $(this).closest("tr").prev().find("th").eq($(this).index()).text(); // 1行上のセルのテキストを取得
            if (name != "")
            {
                alert("被り : " + textAbove + " / " + name);
            }
    ...
    function process(dateArray, end)
    {
        ...
        plans.forEach(function(plan, index) 
        {
            ...
            // console.log("end : " + end + " | endIdx : " +endIdx + " / planIdx : " + planIdx);
    ...
    function auto(dateArray, end)
    {
        ...
        let createCol = $("colgroup");
        // headderを生成しやすくするため、格納
        let headerRow = $(".tbl thead .headder");
        let percentRow = $(".tbl thead .percent");
    ...
    //         let rowData = [];
    //         $(this).find("td").slice(8).each(function() { // 8列目（インデックス7）以降を取得
    //             let nameValue = $(this).attr("name"); // name属性を取得
    //             let lastValue = nameValue ? nameValue.split(" / ").pop() : ""; // nameがない場合は空白
    //             rowData.push(lastValue);
    //         });
    //         tableData.push(rowData);
    //     });
    
    //     // 縦（列）ごとのデータを取得
    //     let numCols = tableData[0]?.length || 0;
    
    //     for (let col = 0; col < numCols; col++) {
    //         let colValues = tableData.map(row => row[col]).filter(val => val !== ""); // 空白を除外
    //         let uniqueValues = new Set(colValues);
    //         let percentIncrease = 0;
    
    //         // 被りがある場合の処理
    //         if (uniqueValues.size < colValues.length) {
    //             $(".tbl thead .percent th").eq(col + 8).css("background-color", "red");
    //         }
    
    //         // workerNames のチェック
    //         let foundNames = new Set();
    //         colValues.forEach(value => {
    //             if (workerNames.includes(value)) {
    //                 foundNames.add(value); // 一度だけカウントするためにSetを使用
    //             }
    //         });
    
    //         // workerNames 内の名前が見つかった分だけ 20% を加算
    //         percentIncrease = foundNames.size * 20;
    
    //         // ヘッダーのテキストを変更
    //         let $header = $(".tbl thead .percent th").eq(col + 8);
    //         let newText = `${percentIncrease}%`;
    //         $header.text(newText);
    //     }
    // }
        ...
        for (let col = 0; col < numCols; col++) {
            ...
            colValues.forEach(value => {
                if (uniqueValues.has(value)) {
                    duplicateValues.add(value); // 重複している値を取得
                } else {
            ...
            if (duplicateValues.size > 0) {
                ...
                $header.attr("name", Array.from(duplicateValues).join(", ")); // 重複値をname属性に設定
            } else {
            ...
            // workerNames 内の名前が見つかった分だけ 20% を加算
            let percentIncrease = foundNames.size * 20;
