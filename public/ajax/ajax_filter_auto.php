...
$sql = "SELECT * FROM auto_info WHERE worker = '$worker_name'";
...
// 取得したデータを格納
$auto_info_worker = $stmt->fetchAll(PDO::FETCH_ASSOC);

// データが存在しない場合の処理
if (empty($auto_info_worker)) {
    echo json_encode(['error' => '該当するデータがありません。']);
    exit;
}

// 担当者の含まれる、process_infoのデータが複数ある時
foreach ($auto_info_worker as $auto) 
{
    // 他のデータを取得しやすいように格納
    $excel_info_id = $auto['excel_info_id'];
